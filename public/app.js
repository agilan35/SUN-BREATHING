...
function initEventListeners() {
    ...
    elements.heroHistoryBtn.addEventListener('click', scrollToHistory);
    ...
    elements.sampleRealBtn.addEventListener('click', () => selectPresetSample('real'));
    elements.sampleFakeBtn.addEventListener('click', () => selectPresetSample('fake'));

    // Start Scan Button
    elements.startScanBtn.addEventListener('click', executeScanWorkflow);

    // Report Modal
    elements.closeReportBtn.addEventListener('click', () => closeModal(elements.reportModal));
    elements.btnCloseReport.addEventListener('click', () => closeModal(elements.reportModal));
    elements.viewNormalBtn.addEventListener('click', () => togglePreviewMode('normal'));
    elements.viewHeatmapBtn.addEventListener('click', () => togglePreviewMode('heatmap'));
    elements.btnExportPdf.addEventListener('click', exportPdfReport);
    elements.btnExportJson.addEventListener('click', exportJsonReport);

    // Info Modals
    elements.closeAboutBtn.addEventListener('click', () => closeModal(elements.aboutModal));
    elements.btnCloseAbout.addEventListener('click', () => closeModal(elements.aboutModal));
    elements.closeContactBtn.addEventListener('click', () => closeModal(elements.contactModal));
...
async function fetchScans() {
    try {
        ...
        console.warn('Using local scan data:', err);
    }
}
...
async function fetchFeatures() {
    try {
        ...
        if (json.data && json.data.length) {
            // Updated features from database
        }
    } catch (err) {
